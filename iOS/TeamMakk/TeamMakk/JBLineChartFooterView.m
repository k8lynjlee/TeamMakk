...
- (void)addLabelTitles:(NSMutableArray *)stringTitles
{
  for (NSString *string in stringTitles) {
...
- (void)layoutSubviews
{
  ...
  for (UILabel *label in self.labels) {
    label.frame = CGRectMake(stepLength*count - 2, yOffset, width, self.bounds.size.height);
    count++;
  }
